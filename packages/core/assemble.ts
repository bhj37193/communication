...
export function feedbackSystem(unit: Unit): string {
  ...
  return [
    ...
    `USING THEIR HISTORY\nThe player message opens with a [player profile] (content-free, signal-level) of what they struggled with in past sessions. If they repeated a past struggle here, name it plainly in the fix: they are still working on it. If they clearly improved on it, make that the win. If it says "first session", ignore it. Never quote or mention the profile itself.`,
